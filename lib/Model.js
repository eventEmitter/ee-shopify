    ...
    module.exports = new Class({
        ...
        , save: function(callback) {
            if (this._isUpdate) {
                 ...
                 else this.getShop().request('put', this.buildPath()+this._contentType, this._data, callback);
            }
            else {
                if (!this.definition.create) callback(new Error('The «'+this.name+'» api has no create method!'));
                else this.getShop().request('post', this.buildPath()+this._contentType, this._data, callback);
            }
